    ...
    <label>Class Type:<br>
        <select name="classtime_type" style="width: 100%;">
            ...
            <?php foreach (get_terms(['taxonomy' => 'classtime_type', 'hide_empty' => false]) as $term): ?>
                <option value="<?php echo $term->term_id; ?>" <?php selected(in_array($term->term_id, $class_type)); ?>>
    ...
    <label>Class Level:<br>
        <select name="classtime_level" style="width: 100%;">
            ...
            <?php foreach (get_terms(['taxonomy' => 'classtime_level', 'hide_empty' => false]) as $term): ?>
                <option value="<?php echo $term->term_id; ?>" <?php selected(in_array($term->term_id, $class_level)); ?>>
                    <?php echo esc_html($term->name); ?>
                </option>
            <?php endforeach; ?>
        </select>
...
add_action('save_post', function ($post_id) {
    ...
    update_post_meta($post_id, '_classtime_start', sanitize_text_field($_POST['classtime_start'] ?? ''));
    update_post_meta($post_id, '_classtime_end', sanitize_text_field($_POST['classtime_end'] ?? ''));
    update_post_meta($post_id, '_classtime_notes', sanitize_textarea_field($_POST['classtime_notes'] ?? ''));
    update_post_meta($post_id, '_classtime_recurrence', sanitize_text_field($_POST['classtime_recurrence'] ?? ''));
    update_post_meta($post_id, '_classtime_date', sanitize_text_field($_POST['classtime_date'] ?? ''));
    update_post_meta($post_id, '_classtime_repeat_until', sanitize_text_field($_POST['classtime_repeat_until'] ?? ''));

    // ✅ Save multiple selected days
    if (!empty($_POST['classtime_day_of_week']) && is_array($_POST['classtime_day_of_week'])) {
        $days = array_map('sanitize_text_field', $_POST['classtime_day_of_week']);
